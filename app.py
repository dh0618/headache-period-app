...
# 질문 흐름 처리
def question_flow(sym_type, questions):
    st.title(f"{sym_type} 증상 체크")
    step = st.session_state.step

    q, options = questions[step]
    choice = st.radio(q, options, key=f"{sym_type}_{step}")

    if st.button("다음"):
        st.session_state.answers[f"{sym_type}_{step}"] = choice
        st.session_state.step += 1
        st.experimental_rerun()

# 결과 표시
def show_result():
    sym_type = st.session_state.answers["선택"]
    answers = [st.session_state.answers[f"{sym_type}_{i}"] for i in range(4)]
    med_key = classify(sym_type, answers)
